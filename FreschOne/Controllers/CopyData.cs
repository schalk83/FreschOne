    ...
    public class DataCopyController : Controller
    {
        ...
        private readonly Dictionary<string, string> queries = new()
        {
           // ["account_balance_snapshot_1_2"] = @"SELECT [DateKey], [account_id], [real_balance], [bonus_balance], [snapshot_timestamp], [currency], [snapshot_date], [ModifiedDate], [Operation] FROM account_balance_snapshot_1_2 with (nolock) WHERE DateKey >= 20250201",
            ["slots_latest_1_0"] = @"SELECT [DateKey], [id], [name], [fixed_name], [network], [sub_network], [gameid], [gameid_low], [bonus_gameid], [bonus_mobile_gameid], [mobile_gameid], [mobile_gameid_low], [minigame], [freespins], [freespins_first], [game_type], [leaderboard], [status], [date_added], [wager_requirement], [snapshot_timestamp], [ModifiedDate], [Operation] FROM slots_latest_1_0 WHERE DateKey >= 20250201 AND DateKey < 20250301",
            ["player_event_dims_1_0"] = @"SELECT [DateKey], [user_account], [Opened], [Active], [FirstPlayed], [ModifiedDate], [Operation] FROM player_event_dims_1_0 WHERE DateKey >= 20250201  AND DateKey < 20250301 ",
            ["account_1_2"] = @"SELECT [DateKey], [id], [address_country], [address_region], [registration_date], [currency], [deposit_limit_daily], [deposit_limit_weekly], [deposit_limit_monthly], [deposit_limit_set], [withdraw_limit_daily], [withdraw_limit_weekly], [withdraw_limit_monthly], [subaff_id], [ftd_date], [brand_name], [brand_type], [brand_internal_brand], [snapshot_timestamp], [inserted_on], [ModifiedDate], [Operation] FROM account_1_2 WHERE DateKey >= 20250201  AND DateKey < 20250301",
            ["psp_transaction_1_0"] = @"SELECT [DateKey], [transaction_date], [transaction_timestamp], [user_account], [currency], [description], [category], [transaction_value], [jms_transaction_id], [ledger_transaction_guid], [ModifiedDate], [Operation] FROM psp_transaction_1_0 WHERE DateKey >= 20250201  AND DateKey < 20250301",
            ["psp_brand_transaction_1_0"] = @"SELECT [DateKey], [transaction_sale_date], [sale_date], [request_date], [id], [transaction_type], [user_id], [brand], [currency], [amount], [bonus], [status], [OrderIDReference], [Processor], [ModifiedDate], [Operation] FROM psp_brand_transaction_1_0 WHERE DateKey >= 20250201  AND DateKey < 20250301",
            ["revenue_summary_hourly_1_0"] = @"SELECT [reporting_date], [reporting_datetime], [user_account], [currency], [account_type], [description], [category], [brand], [network], [gamecode], [total_value], [total_transactions], [Datekey], [ModifiedDate], [Operation] FROM revenue_summary_hourly_1_0 WHERE DateKey >= 20250201  AND DateKey < 20250301",
            ["adjustments_summary_hourly_1_0"] = @"SELECT [DateKey], [reporting_date], [user_account], [currency], [account_type], [description], [category], [total_value], [total_transactions], [ModifiedDate], [Operation] FROM adjustments_summary_hourly_1_0 WHERE DateKey >= 20250201  AND DateKey < 20250301",
            ...
            ["CurrencyConversion"] = @"SELECT DateKey,SourceCurrencyID,DestinationCurrencyID,SourceDestinationExchRate FROM dbMasterData..CurrencyConversion WHERE DateKey >= 20250201 and DateKey < 20250301",
            ...
            ["PSP_OrderID_Lookup"] = @"SELECT id, user_id , transaction_type, OrderIDReference, Processor, DateKey,ModifiedDate, Operation, Brand from PSP_OrderID_Lookup where datekey >= 20250201 and DateKey < 20250401
"
        };
        ...
        public IActionResult Index()
        {
            ...
            foreach (var kvp in queries)
            {
                ...
                try
                {
                    var sourceCmd = new SqlCommand($"SELECT COUNT(*) FROM ({kvp.Value}) AS src", sourceConn);
                ...
                try
                {
                    var targetCmd = new SqlCommand($"SELECT COUNT(*) FROM [{kvp.Key}]", targetConn);
